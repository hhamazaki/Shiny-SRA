---
title: "SR Model Analysis Report"
author:
...
# Original Age range 
  ...
  trimage <- ifelse(params$data=='Run'&((min(o.age())-input$rage[1])!=0|(max(o.age())-input$rage[2])!=0),TRUE,FALSE)
# Trimmed Age
  r.age <-ifelse(params$data=='Run',ifelse(trimage,paste(input$rage[1],'-',input$rage[2]),'No Age data trimming'),'NA')
# Age Trimming method  
  pool  <- ifelse(trimage,
...
rbage <-ifelse(params$data =='Run',paste(input$rage[1],'-',input$rage[2]),'NA')  
...
run <- ifelse(params$data =='Run','','')
brood <- paste0(length(data_sr.0()$Yr),'years, ranging from ','')
spr <- ifelse(params$data =='Run','The constructed','The input')
trim <- paste('In this model analysis, the original data were trimmed to','','years, ranging from','','.')
yy <- ifelse(rn.yrs  !=length(data_sr.0()$Yr),'','')
...
# Add parameters for data and model descriptions--------------------------------
  jpar <- params$JAGS
  madd <- ifelse(params$add=='ar1','AR(1) error',ifelse(params$add=='kf','time-variant alpha','None'))
  ss <- ifelse(SS(),'state-space',ifelse(RE(),'Measurement Error','standard'))
...
Year range:  `r runyr`   
Brood Year range: `r byr`  
Age range:   `r obage`   

**Data Modification**  
Trimmed Brood Year range: `r rbyr`  
Trimmed Age range: `r r.age`   
...
```{r }
kable(params$tbl_sumpost,caption = paste0("Spawner Recruit model parameters summary."),digits = c(rep(3,4),0,0,3,0,0),row.names = FALSE) 
```
