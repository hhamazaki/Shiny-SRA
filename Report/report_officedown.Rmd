...
```{r,datadesciption, echo=FALSE,include=FALSE}
  runyr <- paste(min(params$tbl_run$Year),'-',max(params$tbl_run$Year))
  byr <- paste(min(data_sr.0()$Yr),'-',max(data_sr.0()$Yr))
  yrange <- ifelse(params$data =='Run',runyr,byr)
  nyr <- input$sryears[2]-input$sryears[1]+1
  o.yrs <- paste(min(data_sr.0()$Yr),'to',max(data_sr.0()$Yr))
  yrs <- paste(input$sryears[1],'to',input$sryears[2])
  rbyr <- paste(input$sryears[1],'-',input$sryears[2])
  o.age <- c(min(o.age()),max(o.age()))
  r.age <- input$rage 
  obage <-ifelse(params$data =='Run',paste(o.age[1],'-',o.age[2]),'NA')
  rbage <-ifelse(params$data =='Run',paste(input$rage[1],'-',input$rage[2]),'NA')
  pool  <- ifelse(params$data =='Run'& ((o.age[1]-r.age[1])!=0|(o.age[2]-r.age[2])!=0),
