...
```{r }
kable(sumbayes(),caption = paste0("Spawner Recruit model parameters summary.")) 
```
...
```
```{r,echo=FALSE,eval = (input$T_Yld__prof) ,fig.dim = c(18,8),fig.cap = paste0("Target Yield Profile Analyses plot. The line indicate probability of a specified escapement producing yield greater than the target yield.")}
#params$plt_max_prof
```
...
```
```{r,echo=FALSE,eval = (input$T_Rec__prof) ,fig.dim = c(18,8),fig.cap = paste0("Target Recruit Profile Analyses plot. The line indicates a specified escapement producing recruit greater than the target recruit.")}
#params$plt_max_prof
```
